...
describe('Expense Repository', () => {
  ...
  const repositoryMockFactory: () => MockType<Repository<ExpenseEntity>> =
    jest.fn(() => ({
      findOne: jest.fn(() => {
        return {};
      }),
  ...
  it('create should return the created entity', async () => {
    ...
    const expense = await provider.create(
      expensesArr[0] as CreateExpenseDto,
      user1 as UserEntity,
    );
  ...
  it('update should return the updated entity', async () => {
    ...
    repoMock.merge.mockImplementation(() => ({
      ...expensesArr[0],
      ...updateExpenseDto,
    }));
    ...
    const expense = await provider.update(
      expensesArr[0] as ExpenseEntity,
      updateExpenseDto,
    );
